class Pawn < Piece
  ...
  def no_forward_obstruction?(x_des, y_des)
    ...
    if move_type(x_des, y_des) == :vertical &&
       !pieces_in_column(y_des).where('y_position > ? AND y_position < ?',
                               [y_position, y_des.to_i].min,
                               [y_position, y_des.to_i].max).empty?
    end
  end

  # can pawn move 2 squares forward or obstructed
  def move_forward_white?(x_des, y_des)
  ...
  def move_forward_black?(x_des, _y_des)
    ...
    if y_position == 6 && color == Piece::BLACK &&
       move_type(x_des.to_i, 4) == :vertical &&
  ...
  def no_x_move?(x_des)
    ...
    if (pawn_capture_move false) && (x_des.to_i - x_position).zero?
    end
  end
